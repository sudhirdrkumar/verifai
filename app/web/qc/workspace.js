(function () {
  ...
  async function renderUserDashboard() {
    ...
    const assigneeRows = assigneeItems.map(function (item) {
      ...
      const starsText = totalStars > 0
        ? (String(totalStars) + ' star' + (totalStars === 1 ? '' : 's') + (ratedClaims > 0 ? (' (' + String(ratedClaims) + ' rated)') : ''))
        : '0';
      return '<tr>'
  ...
  async function renderDoctorAssignedCases() {
    ...
    contentPanel.innerHTML = '<section class="claim-status-panel">'
      ...
      + '<div class="table-wrap claim-status-table-wrap"><table><thead><tr>'
      + '<th>Claim ID</th><th>Status</th><th>VerifAI JSON</th><th>Documents</th><th>Last Upload</th><th>Assigned At</th><th>Allotment Date</th><th>Final Status</th><th>Action</th>'
    ...
    try {
      ...
      setMessage('doctor-assigned-msg', 'err', err.message);
      tbody.innerHTML = '<tr><td colspan="9">Failed to load assigned cases.</td></tr>';
      updatePaginationUi();
    }
  }
  async function renderDoctorCaseDetail() {
    ...
    function setActionDisabled(disabled) {
      const shouldDisable = !!disabled || !hasVerifaiReportJsonReady();
    ...
    function buildLegacyReportHtml(generatedAt, doctorName, extractionPairs, evidenceLines) {
      ...
      const triggeredConditionNotes = checklistRows
        ...
        .map(function (entry) {
          ...
          if (!detail) return '';
          if (/learning from conclusion and extraction/i.test(detail)) return '';
          if (/triggered condition identified/i.test(detail)) return '';
      ...
      const rejectDetailedFallback = sanitizeConclusionText(
        'The claim is not genuine and is not recommended for approval. Inpatient admission during ' + admissionWindowText + ' (LOS: ' + lengthOfStay + ') for ' + diagnosisForConclusion + ' requires stronger clinical substantiation in submitted records. '
        ...
        + (triggeredReasonText ? ('Rejection basis: ' + summarizeForConclusion(triggeredReasonText, 160) + '. ') : '')
      ...
      try {
        ...
        if (!hasVerifaiReportJsonReady()) {
          ...
          if (waitResult.ready) {
            ...
            renderReportLoadingTab(reportTab, 'VerifAI JSON still not ready.', formatElapsedClock(waitResult.elapsedMs));
            try {
        ...
        if ((coverage.missingDocs || []).length > 0) {
          ...
          const msg = 'VerifAI JSON is required before report generation. Missing AI response for ' + String(missing) + ' document(s).';
  ...
  async function renderUploadDocument() {
    ...
    contentPanel.innerHTML = '<section class="claim-status-panel">'
      ...
      + '<div class="table-wrap claim-status-table-wrap"><table><thead><tr>'
      + '<th>CLAIM ID</th><th>ASSIGNED DOCTOR</th><th>ALLOTMENT DATE</th><th>STATUS</th><th>VERIFAI JSON</th><th>MERGE SUMMARY</th><th>LAST UPLOAD</th><th>ACTION</th>'
      + '</tr></thead><tbody id="upload-doc-tbody"><tr><td colspan="8">Loading...</td></tr></tbody></table></div>'
      + '<div class="claim-pagination">'
  ...
  async function renderCompletedReports(initialStatusFilter) {
    ...
    function buildRatingOptions(selectedValue) {
      ...
      for (let i = 1; i <= 5; i += 1) {
        html += '<option value="' + String(i) + '"' + (i === current ? ' selected' : '') + '>' + String(i) + ' star' + (i === 1 ? '' : 's') + '</option>';
      }
      return html;
    }
      ...
      const rows = items.map(function (r) {
        ...
        const ratingCellHtml = (canAuditQc && isQcDone)
          ? ('<div class="action-row action-row-primary">'
            ...
            + '</select>'
            + '<button type="button" class="btn-soft" data-completed-save-rating="' + escapeHtml(claimUuid) + '">Save</button>'
            + '</div>')
          : (isQcDone
