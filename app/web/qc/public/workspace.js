(function () {
  ...
  const NAV = {
    super_admin: [
      ...
      { page: "doctor-ratings", label: "Doctor Ratings" },
  ...
  function ratingQualityLabel(value) {
    const n = Number(value || 0);
    if (!Number.isFinite(n) || n <= 0) return 'No rating';
    if (n < 3) return 'Poor quality';
    if (n < 4) return 'Average quality';
    if (n < 5) return 'Good quality';
    return 'Excellent';
  }

  function qualityHtml(label) {
    const text = String(label || '').trim() || 'No rating';
    if (text.toLowerCase() === 'poor quality') {
      return '<span style="color:#b42318;font-weight:700;">' + escapeHtml(text) + '</span>';
    }
  ...
  function statusChip(status) {
    ...
    let cls = "info";
    if (["completed", "uploaded", "approve", "approved", "active", "yes", "succeeded", "received"].includes(s)) cls = "success";
    if (["withdrawn", "reject", "rejected", "failed", "hard_reject"].includes(s)) cls = "warn";
    if (["pending", "processing", "inactive", "no", "waiting", "not_sent"].includes(s)) cls = "muted";
    return '<span class="status-chip ' + cls + '">' + escapeHtml(status || "-") + "</span>";
  }

  function buildVerifaiJsonCellHtml(state, metaText) {
    const normalizedState = String(state || 'not_sent').trim().toLowerCase();
    const label = normalizedState === 'received'
      ? 'Received'
      : (normalizedState === 'waiting' ? 'Waiting' : 'Not Sent');
    const meta = String(metaText || '').trim();
    return statusChip(label) + (meta ? ('<div class="muted" style="margin-top:4px;">' + escapeHtml(meta) + '</div>') : '');
  }

  function renderVerifaiJsonStatusCell(item, options) {
    const opts = options || {};
    const claimUuid = String((item && item.id) || (item && item.claim_uuid) || '').trim();
    const caseId = String((item && item.verifai_case_id) || '').trim();
    const state = String((item && item.verifai_json_state) || (caseId ? 'waiting' : 'not_sent')).trim().toLowerCase();
    const meta = [String((item && item.verifai_status) || '').trim(), String((item && item.verifai_stage) || '').trim()].filter(Boolean).join(' / ');
    const attrs = [
      'data-verifai-json-cell="1"',
      'data-claim-uuid="' + escapeHtml(claimUuid) + '"',
      'data-case-id="' + escapeHtml(caseId) + '"',
      'data-json-state="' + escapeHtml(state) + '"',
      'data-meta="' + escapeHtml(meta) + '"',
    ];
    if (opts.compact) attrs.push('data-compact="1"');
    return '<td ' + attrs.join(' ') + '>' + buildVerifaiJsonCellHtml(state, meta) + '</td>';
  }

  function setVerifaiJsonCellState(cell, state, metaText) {
    if (!cell) return;
    const normalizedState = String(state || 'not_sent').trim().toLowerCase();
    const meta = String(metaText || '').trim();
    cell.setAttribute('data-json-state', normalizedState);
    cell.setAttribute('data-meta', meta);
    cell.innerHTML = buildVerifaiJsonCellHtml(normalizedState, meta);
  }

  async function refreshVerifaiJsonStatusCells(rootEl) {
    const scope = rootEl || document;
    const cells = Array.from(scope.querySelectorAll('td[data-verifai-json-cell="1"]'));
    if (!cells.length) return;

    const targets = cells.filter(function (cell) {
      const claimUuid = String(cell.getAttribute('data-claim-uuid') || '').trim();
      const caseId = String(cell.getAttribute('data-case-id') || '').trim();
      return !!claimUuid && !!caseId;
    });
    ...
    async function refreshOne(cell) {
      const claimUuid = String(cell.getAttribute('data-claim-uuid') || '').trim();
      const currentMeta = String(cell.getAttribute('data-meta') || '').trim();
      if (!claimUuid) return;
      try {
        const payload = await apiFetch('/api/v1/integrations/verifai/claims/' + encodeURIComponent(claimUuid) + '/report-json?ts=' + String(Date.now()));
        const hasReport = !!(payload && typeof payload.report_json === 'object' && payload.report_json && Object.keys(payload.report_json).length > 0);
        const meta = [String((payload && payload.verifai_status) || '').trim(), String((payload && payload.verifai_stage) || '').trim()].filter(Boolean).join(' / ');
        setVerifaiJsonCellState(cell, hasReport ? 'received' : 'waiting', meta || currentMeta);
      } catch (err) {
        const msg = String(err && err.message ? err.message : err || '').toLowerCase();
        if (msg.indexOf('not been queued') >= 0) {
          setVerifaiJsonCellState(cell, 'not_sent', '');
        } else {
          setVerifaiJsonCellState(cell, 'waiting', currentMeta);
        }
      }
    }
    ...
    async function worker() {
      while (cursor < targets.length) {
        const index = cursor;
        cursor += 1;
        await refreshOne(targets[index]);
      }
    }
  ...
  function renderClaimsTable(items) {
    ...
    const rows = items.map((item) => {
      return "<tr>"
        ...
        + renderVerifaiJsonStatusCell(item, { compact: true })
        + "<td>" + escapeHtml(item.assigned_doctor_id || "-") + "</td>"
        + "<td>" + escapeHtml(formatDateTime(item.created_at)) + "</td>"
        + "</tr>";
    }).join("");
    return '<div class="table-wrap"><table><thead><tr><th>Claim ID</th><th>Patient</th><th>Status</th><th>VerifAI JSON</th><th>Doctor</th><th>Created</th></tr></thead><tbody>' + rows + "</tbody></table></div>";
  }
  ...
  async function renderDoctorDashboard() {
    ...
    const currentUsername = String((me && me.username) || '').trim();
    const doctorId = (me && me.role === 'doctor')
      ? currentUsername
      : (doctorFromQuery || currentUsername);
    ...
    const [allSummary, completedSummary, withdrawnSummary, listPayload, openClaimsPayload, completionStats, doctorRatingOverview] = await Promise.all([
      ...
      apiFetch('/api/v1/user-tools/doctor-rating-overview').catch(function () {
        return { summary: {} };
      }),
    ]);
    ...
    const ratingSummary = doctorRatingOverview && doctorRatingOverview.summary ? doctorRatingOverview.summary : {};
    const averageRating = Number((ratingSummary && ratingSummary.average_rating) || 0);
    const ratedCases = Number((ratingSummary && ratingSummary.rated_cases) || 0);
    const averageQuality = ratingQualityLabel(averageRating);
    ...
    const visibleItems = (function () {
      ...
      if (orderedOpenItems.length > 0) {
        return orderedOpenItems.map(function (item) {
          ...
          return {
            id: String(item.id || base.id || '').trim(),
            ...
            verifai_case_id: String(item.verifai_case_id || '').trim(),
            verifai_status: String(item.verifai_status || '').trim(),
            verifai_stage: String(item.verifai_stage || '').trim(),
            verifai_json_state: String(item.verifai_json_state || '').trim(),
          };
        });
      }
    ...
    contentPanel.innerHTML = '<h2>Dashboard Overview</h2><p class="muted">Quick summary of your assigned and completed workload.</p>'
      ...
      + '<article class="stat-card"><div class="muted">Average Rating</div><div class="value">' + escapeHtml(averageRating.toFixed(2)) + '</div><div class="muted">' + escapeHtml(String(ratedCases)) + ' rated case(s)</div><div class="muted">Quality: ' + qualityHtml(averageQuality) + '</div></article>'
      + '</div>'
    ...
    await refreshVerifaiJsonStatusCells(contentPanel);
  ...
  async function renderUserDashboard() {
    ...
    const assigneeRows = assigneeItems.map(function (item) {
      const totalStars = Number(item && item.auditor_total_stars ? item.auditor_total_stars : 0);
      const ratedClaims = Number(item && item.rated_completed_count ? item.rated_completed_count : 0);
      const starsText = totalStars > 0
        ? (String(totalStars) + ' stars' + (ratedClaims > 0 ? (' (' + String(ratedClaims) + ' rated)') : ''))
        : '0';
      return '<tr>'
    ...
    contentPanel.innerHTML = '<h2>Dashboard Overview</h2><p class="muted">Day-wise completed and pending status across assigned cases.</p>'
      ...
      + '<div class="table-wrap"><table><thead><tr><th>User</th><th>Role</th><th>Completed</th><th>Pending</th><th>Total</th><th>Auditor Stars</th></tr></thead><tbody>'
      + (assigneeRows || '<tr><td colspan="6">No assignee data found.</td></tr>')
      + '</tbody></table></div>';
  }

  async function renderDoctorRatings() {
    const payload = await apiFetch('/api/v1/user-tools/doctor-rating-overview').catch(function () {
      return { summary: {}, items: [] };
    });
    const summary = payload && payload.summary ? payload.summary : {};
    const averageRating = Number(summary && summary.average_rating ? summary.average_rating : 0);
    const averageQuality = ratingQualityLabel(averageRating);
    const totalStars = Number(summary && summary.total_stars ? summary.total_stars : 0);
    const completedCases = Number(summary && summary.completed_cases ? summary.completed_cases : 0);
    const ratedCases = Number(summary && summary.rated_cases ? summary.rated_cases : 0);

    const rows = [5, 4, 3, 2, 1].map(function (star) {
      const count = Number(summary && summary['rating_' + String(star) + '_count'] ? summary['rating_' + String(star) + '_count'] : 0);
      return '<tr>'
        + '<td>' + escapeHtml(String(star)) + '</td>'
        + '<td>' + escapeHtml(String(count)) + '</td>'
        + '</tr>';
    }).join('');

    contentPanel.innerHTML = '<h2>My Ratings</h2><p class="muted">Average rating and star-wise distribution from auditor feedback.</p>'
      + '<div class="stats-grid">'
      + '<article class="stat-card"><div class="muted">Average Rating</div><div class="value">' + escapeHtml(averageRating.toFixed(2)) + '</div></article>'
      + '<article class="stat-card"><div class="muted">Quality</div><div class="value">' + qualityHtml(averageQuality) + '</div></article>'
      + '<article class="stat-card"><div class="muted">Total Stars</div><div class="value">' + escapeHtml(String(totalStars)) + '</div></article>'
      + '<article class="stat-card"><div class="muted">Rated Cases</div><div class="value">' + escapeHtml(String(ratedCases)) + '</div></article>'
      + '<article class="stat-card"><div class="muted">Completed Cases</div><div class="value">' + escapeHtml(String(completedCases)) + '</div></article>'
      + '</div>'
      + '<h3 style="margin-top:18px">Star Distribution</h3>'
      + '<div class="table-wrap"><table><thead><tr><th>Rating</th><th>Cases</th></tr></thead><tbody>'
      + (rows || '<tr><td colspan="2">No rating data found.</td></tr>')
      + '</tbody></table></div>';
  }

  async function renderUserDoctorRatings() {
    const payload = await apiFetch('/api/v1/user-tools/doctor-rating-overview').catch(function () {
      return { items: [] };
    });
    const items = (Array.isArray(payload && payload.items) ? payload.items : []).filter(function (item) {
      return Number(item && item.rated_cases ? item.rated_cases : 0) > 0;
    });
    const rows = items.map(function (item) {
      const averageRating = Number(item && item.average_rating ? item.average_rating : 0);
      const qualityLabel = ratingQualityLabel(averageRating);
      return '<tr>'
        + '<td>' + escapeHtml(String(item && item.doctor_username ? item.doctor_username : '-')) + '</td>'
        + '<td>' + escapeHtml(formatRoleLabelGlobal(item && item.role ? item.role : '')) + '</td>'
        + '<td>' + escapeHtml(String(item && item.total_stars ? item.total_stars : 0)) + '</td>'
        + '<td>' + escapeHtml(averageRating.toFixed(2)) + '</td>'
        + '<td>' + qualityHtml(qualityLabel) + '</td>'
        + '<td>' + escapeHtml(String(item && item.rated_cases ? item.rated_cases : 0)) + '</td>'
        + '<td>' + escapeHtml(String(item && item.completed_cases ? item.completed_cases : 0)) + '</td>'
        + '<td>' + escapeHtml(String(item && item.rating_5_count ? item.rating_5_count : 0)) + '</td>'
        + '<td>' + escapeHtml(String(item && item.rating_4_count ? item.rating_4_count : 0)) + '</td>'
        + '<td>' + escapeHtml(String(item && item.rating_3_count ? item.rating_3_count : 0)) + '</td>'
        + '<td>' + escapeHtml(String(item && item.rating_2_count ? item.rating_2_count : 0)) + '</td>'
        + '<td>' + escapeHtml(String(item && item.rating_1_count ? item.rating_1_count : 0)) + '</td>'
        + '</tr>';
    }).join('');

    contentPanel.innerHTML = '<h2>Doctor Ratings</h2><p class="muted">Doctor-wise total and average rating with star distribution.</p>'
      + '<div class="table-wrap"><table><thead><tr>'
      + '<th>Assignee</th><th>Role</th><th>Total Stars</th><th>Average</th><th>Quality</th><th>Rated</th><th>Completed</th><th>5 star</th><th>4 star</th><th>3 star</th><th>2 star</th><th>1 star</th>'
      + '</tr></thead><tbody>'
      + (rows || '<tr><td colspan="12">No rated assignee data found.</td></tr>')
      + '</tbody></table></div>';
  }
  ...
  async function renderPaymentSheet() {
    ...
    contentPanel.innerHTML = '<h2>Payment Sheet</h2><p class="muted">Previous month payment sheet based on completed cases and configured rate.</p>'
      ...
      + '<div class="claim-filter-action"><button type="button" class="btn-soft" id="payment-sheet-export-btn">Export Excel</button></div>'
      + '</form>'
    ...
    const exportBtnEl = document.getElementById('payment-sheet-export-btn');
    ...
    function buildPaymentSheetQuery() {
      ...
      return 'month=' + encodeURIComponent(monthVal) + '&include_zero_cases=' + encodeURIComponent(includeZero ? 'true' : 'false');
    }

    async function loadSheet() {
      const qs = buildPaymentSheetQuery();
      if (!qs) return;
      try {
    ...
    async function exportSheet() {
      ...
      try {
        setMessage('payment-sheet-msg', '', 'Preparing Excel export...');
        const fileResult = await apiFetchFile('/api/v1/user-tools/payment-sheet/export?' + qs);
        const a = document.createElement('a');
        a.href = URL.createObjectURL(fileResult.blob);
        a.download = String(fileResult.filename || ('payment_sheet_' + String((monthEl && monthEl.value) || 'sheet') + '.xlsx'));
        ...
        setMessage('payment-sheet-msg', 'ok', 'Payment sheet exported.');
      } catch (err) {
        setMessage('payment-sheet-msg', 'err', err && err.message ? err.message : 'Failed to export payment sheet.');
      } finally {
  ...
  async function renderAssignCases() {
    ...
    contentPanel.innerHTML = '<section class="claim-status-panel">'
      ...
      + '<button type="button" class="btn-soft" id="claim-bulk-withdraw-btn">Withdraw Selected Cases</button>'
      + '</div>'
      + '<p id="assign-msg"></p>'
    ...
    const bulkWithdrawBtn = document.getElementById('claim-bulk-withdraw-btn');
    ...
    async function withdrawClaims(claimIds) {
      if (!claimIds.length) {
        setMessage('assign-msg', 'err', 'Select at least one claim.');
        return;
      }
      const ok = window.confirm('Move selected case(s) to Withdrawn?');
      if (!ok) return;
      ...
      for (const claimId of claimIds) {
        try {
          await apiFetch('/api/v1/claims/' + encodeURIComponent(claimId) + '/status', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ status: 'withdrawn', note: 'withdrawn by user from assign cases' }),
          });
          successCount += 1;
        } catch (_err) {
          failed.push(claimId);
      ...
      if (!failed.length) {
        setMessage('assign-msg', 'ok', successCount + ' case(s) moved to Withdrawn.');
      } else {
        setMessage('assign-msg', 'err', successCount + ' moved, ' + failed.length + ' failed.');
      }
    ...
    async function loadRows(resetPage) {
      ...
      const rows = (result.items || []).map((c) => {
        ...
        return '<tr>'
          ...
          + '<td><div class="link-row"><button type="button" class="btn-soft claim-status-action-btn" data-assign-claim="' + escapeHtml(c.id) + '">Assign</button><button type="button" class="btn-soft claim-status-action-btn" data-withdraw-claim="' + escapeHtml(c.id) + '">Withdraw</button></div></td>'
          + '</tr>';
      }).join('');
      ...
      tbody.querySelectorAll('button[data-withdraw-claim]').forEach((btn) => {
        btn.addEventListener('click', async function () {
          const claimId = String(this.getAttribute('data-withdraw-claim') || '').trim();
          if (!claimId) return;
          await withdrawClaims([claimId]);
        });
      });
    ...
    bulkWithdrawBtn.addEventListener('click', async function () {
      setMessage('assign-msg', '', '');
      await withdrawClaims(getSelectedClaimIds());
    });
  ...
  async function renderDoctorAssignedCases() {
    ...
    const doctorFromQuery = String(pageUrl.searchParams.get('doctor_username') || '').trim();
    const currentUsername = String((me && me.username) || '').trim();
    const scopedDoctor = (me && me.role === 'doctor')
      ? currentUsername
      : (doctorFromQuery || currentUsername);
    ...
    async function loadRows(resetPage) {
      ...
      params.set('exclude_withdrawn', 'true');
      if (scopedDoctor) params.set('doctor_filter', scopedDoctor);
  ...
  async function renderDoctorCaseDetail() {
    ...
    contentPanel.innerHTML = '<section class="claim-status-panel case-detail-panel">'
      ...
      + '<h3 style="margin-top:16px;">VerifAI Doctor Bucket</h3>'
      + '<div id="case-verifai-report" class="case-generated-report muted">Waiting for VerifAI queue/report.</div>'
    ...
    const verifaiReportEl = document.getElementById('case-verifai-report');
    ...
    const actionButtons = [reportBtn, statusBtn].filter(Boolean);

    function ensureNoAiInputModal() {
      let modalEl = document.getElementById('no-ai-input-modal');
      if (modalEl) return modalEl;
      const wrapper = document.createElement('div');
      wrapper.innerHTML = ''
        + '<div id="no-ai-input-modal" class="modal-backdrop">'
        + '<div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="no-ai-input-modal-title">'
        + '<div class="modal-header"><h3 id="no-ai-input-modal-title">No AI Input</h3><button type="button" class="btn-soft" id="no-ai-input-modal-close">Close</button></div>'
        + '<p class="muted" id="no-ai-input-modal-subtitle">VerifAI JSON is missing or too thin for report generation.</p>'
        + '<div id="no-ai-input-modal-body" style="line-height:1.6;color:#1f2937;white-space:pre-wrap;">Please rerun extraction in VerifAIengine and try again.</div>'
        + '<div style="display:flex;justify-content:flex-end;gap:10px;margin-top:18px;">'
        + '<button type="button" id="no-ai-input-modal-ok">OK</button>'
        + '</div>'
        + '</div>'
        + '</div>';
      document.body.appendChild(wrapper.firstChild);
      modalEl = document.getElementById('no-ai-input-modal');
      const closeModal = function () {
        if (modalEl) modalEl.classList.remove('open');
      };
      const closeBtn = document.getElementById('no-ai-input-modal-close');
      const okBtn = document.getElementById('no-ai-input-modal-ok');
    ...
    function showNoAiInputModal(reasonText) {
      const modalEl = ensureNoAiInputModal();
      const subtitleEl = document.getElementById('no-ai-input-modal-subtitle');
      const bodyEl = document.getElementById('no-ai-input-modal-body');
      const claimText = String((currentClaim && currentClaim.external_claim_id) || routeClaimId || '-').trim() || '-';
      if (subtitleEl) subtitleEl.textContent = 'Claim ID: ' + claimText;
      if (bodyEl) {
        bodyEl.textContent = String(reasonText || 'VerifAI did not return enough AI input to generate a reliable report. Please rerun extraction in VerifAIengine and try again.');
      }
      if (modalEl) modalEl.classList.add('open');
    }

    function isMeaningfulAiText(value) {
      const textValue = sanitizeReportText(value);
      if (!textValue || textValue === '-') return false;
      if (/^(?:not available|not provided|none|nil|null|unknown)$/i.test(textValue)) return false;
      if (/^(?:claim reviewed based on available records\.?|query \+ ir for documents\.?|no deranged investigation values found\.?|no medicine list extracted\.?|no date-wise investigation reports available\.?|no date-wise tpr .* extracted\.?)$/i.test(textValue)) return false;
      return true;
    }

    function isDiagnosisCodeOnly(value) {
      const textValue = sanitizeReportText(value).toUpperCase();
      if (!textValue || textValue === '-') return false;
      return /^[A-Z][0-9]{2}(?:\.[0-9A-Z]+)?$/.test(textValue);
    }

    function getVerifaiAiInputState() {
      ...
      if (!report) {
        return {
          ready: false,
          thin: true,
          reason: 'No AI input was received from VerifAI for this claim yet. Please rerun extraction in VerifAIengine and try again.'
        };
      }
      ...
      const investigations = sanitizeReportText(report.investigation_finding_in_details || '');
      const doctor = sanitizeReportText(report.treating_doctor || '');
      const doctorReg = sanitizeReportText(report.treating_doctor_registration_number || '');
      const justification = sanitizeReportText(report.clinical_justification || '');
      const supportingFindings = Array.isArray(report.supporting_findings) ? report.supporting_findings.filter(function (item) {
        return isMeaningfulAiText(item);
      }) : [];
      const score = [
        isMeaningfulAiText(complaints),
        isMeaningfulAiText(findings),
        isMeaningfulAiText(medicines),
        isMeaningfulAiText(investigations),
        isMeaningfulAiText(doctor),
      ...
      const thinBecauseCodeOnly = isDiagnosisCodeOnly(diagnosis)
        ...
        && !isMeaningfulAiText(doctor)
        && supportingFindings.length === 0;
      const isThin = score < 2 || thinBecauseCodeOnly;
      return {
        ready: true,
        thin: isThin,
        reason: isThin
          ? 'No AI input is available for reliable reporting. VerifAI returned only thin fields such as diagnosis code / empty complaints / empty findings / empty medicines. Please rerun extraction in VerifAIengine and try again.'
          : ''
      };
    }
    ...
    function hasVerifaiReportJsonReady() {
      const payload = currentVerifaiReport || {};
    ...
    async function resolveDiagnosisForChecklist() {
      const payload = currentVerifaiReport || {};
      const report = payload && typeof payload.report_json === 'object' && payload.report_json ? payload.report_json : null;
      const diagnosis = sanitizeReportText((report && report.diagnosis) || '');
    ...
    function sanitizeConclusionText(value) {
      ...
      const lines = base.split('\n').map(function (line) {
        ...
        if (/^\s*missing evidence\s*:/i.test(t)) return '';
        if (/\bgst guardrail triggered\b/i.test(t)) return '';
        if (/\bchecklist condition matched and evidence pattern triggered\b/i.test(t)) return '';
        if (/\btriggered condition identified\b/i.test(t)) return '';
        if (/auto-learned rule from auditor conclusion/i.test(t)) return '';
        if (/super-admin approval required/i.test(t)) return '';
        if (/suggested outcome\s*:/i.test(t)) return '';
        if (/\b(?:investigation_finding_in_details|auditor_conclusion|decision_conclusion)\b/i.test(t)) return '';
        if (/^(?:investigation_finding_in_details|auditor_conclusion|decision_conclusion|complaints)(?:\s*,\s*(?:investigation_finding_in_details|auditor_conclusion|decision_conclusion|complaints))*$/i.test(t)) return '';
        ...
        t = t
          ...
          .replace(/\bTriggered condition identified\b[^.;\n]*(?:[.;]|$)/gi, ' ')
          .replace(/\btherefore,\s*one or more clinical rules are triggered\.?/gi, '')
          .replace(/\btherefore,\s*this rule is triggered\.?/gi, '')
          .replace(/\b(?:one or more\s+)?clinical rules?\s+are\s+triggered\.?/gi, '')
          .replace(/\bthis rule is triggered\.?/gi, '')
          .replace(/\btriggers this exclusion\/review rule\.?/gi, 'materially impacts claim assessment.')
          .replace(/\bAuto-learned rule from auditor conclusion and extraction context\.?/gi, ' ')
          .replace(/\bSuper-admin approval required\.?/gi, ' ')
          .replace(/\bSuggested outcome\s*:\s*[A-Z_]+\.?/gi, ' ')
          .replace(/\s{2,}/g, ' ')
          .trim();
        if (!t) return '';
    ...
    function summarizeForConclusion(value, maxLen) {
      ...
      let clipped = t.slice(0, limit).trim();
      ...
      if (softBreak >= Math.floor(limit * 0.6)) {
        clipped = clipped.slice(0, softBreak).trim();
      }
    ...
    function extractVitalsFromLine(value) {
      ...
      const tempRegex = /\b(?:temp(?:erature)?|t)\b\s*[:=\-]?\s*([0-9]{2,3}(?:\.[0-9]{1,2})?)\s*(?:ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°?\s*[CF])?/ig;
    ...
    function buildMedicineEvidenceSummary(primaryMedicineText, extractionPairs) {
      ...
      allTexts.forEach(function (block) {
        String(block || '').split(/\r?\n|;/).forEach(function (line) {
          const cleaned = sanitizeReportText(line).replace(/^[\-ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢\d.()]+\s*/, '').trim();
    ...
    function isRuleCodeRelevantForContext(code, context) {
      ...
      if (!/^R\d{3}$/.test(key)) return false;
    ...
    function buildTriggeredRuleSummary(checklistRows, context) {
      ...
      rows.forEach(function (entry) {
        ...
        const entryName = sanitizeConclusionText(entry.name || entry.title || '');
        if (/learning from conclusion and extraction/i.test(entryName)) return;
    ...
    function getRuleConclusionLineByCode(code, context) {
      ...
      const MAP = {
        R001: 'Use of ' + abxLabel + ' is not supported by sepsis markers or objective evidence of severe infection.',
    ...
    function getDecisionOutcomeForConclusion(finalDecision) {
      ...
      if (d === 'INADMISSIBLE') return { verdict: 'REJECTED', terminalSentence: 'Therefore, the claim is not genuine and is not recommended for approval.' };
    ...
    function buildRuleDrivenConclusion(checklistRows, context) {
      ...
      const finalParagraph = sanitizeConclusionText(
        'Conclusion: The claim has been reviewed against available clinical records. '
    ...
    function buildExactReasonText(decision, context) {
      ...
      if (decision === 'INADMISSIBLE') {
        reasons.push('The claim is not genuine and is not recommended for approval because admission during ' + windowText + ' (LOS: ' + losText + ') for ' + diagnosis + ' requires stronger clinical substantiation in submitted records.');
        if (triggerTextFinal) {
          reasons.push('Key gaps: ' + triggerTextFinal + '.');
        } else if (triggeredRuleSummary) {
          reasons.push('Clinical review indicates documentation gaps.');
        }
        ...
        if (triggerTextFinal) {
          ...
          reasons.push('Clinical review requires additional documentation.');
        }
      }
    ...
    function buildLegacyReportHtml(generatedAt, doctorName, extractionPairs, evidenceLines) {
      ...
      const clinicalFindingsText = (function () {
        ...
        clinicalFindingCandidates.forEach(function (block) {
          String(block || '').split(/\r?\n/).forEach(function (line) {
            const t = sanitizeReportText(line).replace(/^[\-ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢]+\s*/, '').trim();
            if (!t) return;
      ...
      const structuredJustificationText = sanitizeConclusionText(extractTextByAliases(extractionPairs, ['clinical_justification', 'justification'], 2).join('\n'));
      ...
      const triggeredConditionNotesRaw = checklistRows
        .filter(function (entry) {
          ...
          const entryName = sanitizeConclusionText(entry.name || entry.title || '');
          if (/learning from conclusion and extraction/i.test(entryName)) return false;
          const code = extractRuleCodeFromEntry(entry);
      ...
      const rejectDetailedFallback = sanitizeConclusionText(
        'The claim is not genuine and is not recommended for approval. '
        + 'The patient appears to have been managed for ' + diagnosisForConclusion + ' during ' + admissionWindowText + ' (LOS: ' + lengthOfStay + '). '
        + 'Submitted records do not establish adequate clinical substantiation for approval. '
        ...
        + (triggeredRuleSummaryText ? ('Rejection basis: ' + summarizeForConclusion(triggeredRuleSummaryText, 180) + '. ') : '')
      ...
      let recommendationText = firstNonEmpty(structuredRecommendationText, checklistRecommendationText, '');
      if (!recommendationText) {
        if (finalRecommendationDecision === 'ADMISSIBLE') {
          recommendationText = 'Justified + Admissible';
        } else if (finalRecommendationDecision === 'INADMISSIBLE') {
          recommendationText = 'Not Justified + Inadmissible';
        } else {
          recommendationText = 'Not Justified + Inadmissible';
        }
      }
      ...
      const hasStructuredConclusionText = Boolean(String(structuredConclusionText || '').trim());
      const preferredConclusionText = hasStructuredConclusionText
        ? ''
        : firstNonEmpty(ruleDrivenConclusionText, ruleStyleConclusionText, '');
    ...
    function buildReportPairsFromStructuredData(data) {
      ...
      const rawDecisionPayload = (data.raw_payload && data.raw_payload.decision && data.raw_payload.decision.decision_payload && typeof data.raw_payload.decision.decision_payload === 'object')
        ? data.raw_payload.decision.decision_payload
        : {};
      const rawFinalRecommendation = String(rawDecisionPayload.final_recommendation || '').trim().toLowerCase();
      let finalRec = 'QUERY';
      if (rawFinalRecommendation === 'approve' || rawFinalRecommendation === 'admissible') finalRec = 'ADMISSIBLE';
      else if (rawFinalRecommendation === 'reject' || rawFinalRecommendation === 'inadmissible') finalRec = 'INADMISSIBLE';
      else if (rawFinalRecommendation === 'pend_for_documents' || rawFinalRecommendation === 'review' || rawFinalRecommendation === 'investigate' || rawFinalRecommendation === 'query') finalRec = 'QUERY';
      else if (/rejection|reject|inadmissible|not\s+justified/.test(recommendationText)) finalRec = 'INADMISSIBLE';
    ...
    async function refreshCurrentVerifaiReport(options) {
      ...
      const payload = currentVerifaiReport || {};
      const statusItem = currentStatusItem || {};
      const knownCaseId = String((statusItem && statusItem.verifai_case_id) || (payload && payload.verifai_case_id) || '').trim();

      if (!claimUuidText || (!knownCaseId && !hasVerifaiReportJsonReady())) {
        renderVerifaiDoctorBucket();
        if (syncActions) setActionDisabled(false);
        return currentVerifaiReport;
      }

      const previousPayload = currentVerifaiReport;
      const query = new URLSearchParams();
      if (opts.cacheBust !== false) query.set('ts', String(Date.now()));
      ...
      try {
        ...
        currentVerifaiReport = opts.keepPreviousOnError === false ? null : previousPayload;
        renderVerifaiDoctorBucket();
        if (syncActions) setActionDisabled(false);
        throw err;
      }
    }
    async function waitForVerifaiReportJson(options) {
      const opts = options || {};
      ...
      if (!knownCaseId && !hasVerifaiReportJsonReady()) {
        return { ready: false, attempts: 0, elapsedMs: 0, lastError: null };
      }
      ...
      while ((Date.now() - startedAt) < timeoutMs) {
        ...
        if (hasVerifaiReportJsonReady()) {
          return { ready: true, attempts: attempt, elapsedMs: elapsedMs, lastError: null };
        }
      ...
      return {
        ready: hasVerifaiReportJsonReady(),
        attempts: attempt,
        elapsedMs: Date.now() - startedAt,
        lastError: lastError,
      };
    }
    function renderVerifaiDoctorBucket() {
      if (!verifaiReportEl) return;

      const statusItem = currentStatusItem || {};
      const payload = currentVerifaiReport || {};
      const report = payload && typeof payload.report_json === 'object' && payload.report_json ? payload.report_json : null;
      const effectiveCaseId = String((payload && payload.verifai_case_id) || (statusItem && statusItem.verifai_case_id) || '').trim();
      const responseReceived = Boolean(report);
      const sentToVerifai = Boolean(effectiveCaseId || responseReceived);
      const queueStatus = String((payload && payload.verifai_status) || (statusItem && statusItem.verifai_status) || (responseReceived ? 'processed' : sentToVerifai ? 'queued' : '-')).trim() || '-';
      const queueStage = String((payload && payload.verifai_stage) || (statusItem && statusItem.verifai_stage) || (responseReceived ? 'completed' : sentToVerifai ? 'queued' : '-')).trim() || '-';
      const queuedAt = String((payload && payload.verifai_queued_at) || (statusItem && statusItem.verifai_queued_at) || '').trim();

      const badge = function (text, tone) {
        const palette = tone === 'success'
          ? 'background:#dcfce7;color:#166534;border:1px solid #86efac;'
          : tone === 'warning'
            ? 'background:#fef3c7;color:#92400e;border:1px solid #fcd34d;'
            : 'background:#e5e7eb;color:#374151;border:1px solid #d1d5db;';
        return '<span style="display:inline-flex;align-items:center;padding:4px 10px;border-radius:999px;font-size:12px;font-weight:700;' + palette + '">' + escapeHtml(String(text || '-')) + '</span>';
      };

      const infoRow = function (label, valueHtml) {
        return '<div style="display:flex;justify-content:space-between;gap:12px;padding:10px 0;border-top:1px solid #e5e7eb;">'
          + '<div style="font-weight:600;color:#374151;">' + escapeHtml(label) + '</div>'
          + '<div style="text-align:right;color:#111827;">' + valueHtml + '</div>'
          + '</div>';
      };

      const lines = [];
      lines.push(infoRow('Sent To VerifAI', badge(sentToVerifai ? 'Yes' : 'No', sentToVerifai ? 'success' : 'warning')));
      lines.push(infoRow('Response Received', badge(responseReceived ? 'Yes' : 'No', responseReceived ? 'success' : 'warning')));
      lines.push(infoRow('VerifAI Case ID', '<span style="font-weight:600;">' + escapeHtml(effectiveCaseId || '-') + '</span>'));
      lines.push(infoRow('VerifAI Queue Status', badge(queueStatus, responseReceived ? 'success' : sentToVerifai ? 'warning' : 'muted')));
      lines.push(infoRow('VerifAI Queue Stage', '<span style="font-weight:600;">' + escapeHtml(queueStage) + '</span>'));
      lines.push(infoRow('VerifAI Queued At', '<span style="font-weight:600;">' + escapeHtml(queuedAt ? formatDateTime(queuedAt) : '-') + '</span>'));

      verifaiReportEl.className = 'case-generated-report';
      verifaiReportEl.innerHTML = '<div style="border:1px solid #c7d2fe;border-radius:16px;padding:16px 18px;background:linear-gradient(180deg,#f8fbff 0%,#eef6ff 100%);box-shadow:0 8px 20px rgba(15,23,42,0.06);">'
        + '<div style="font-size:14px;font-weight:800;color:#1d4ed8;margin-bottom:6px;">VerifAI Status</div>'
        + '<div style="font-size:12px;color:#64748b;margin-bottom:8px;">Simple handoff status from qc-python to VerifAI.</div>'
        + lines.join('')
        + '</div>';
    }
    function renderGeneratedReport() {
      if (!generatedReportEl) return;
      if (currentClaim && !hasUsableVerifaiAiInput()) {
        generatedReportEl.className = 'case-generated-report muted';
        generatedReportEl.innerHTML = 'No AI input from VerifAI yet.';
        return;
      }
    ...
    async function loadSavedReportBySource(source, silentIfMissing, options) {
      ...
      try {
        ...
        if (!hasUsableVerifaiAiInput()) {
          appendLog((targetSource === 'system' ? 'System' : 'Doctor') + ' saved report skipped because VerifAI input is missing or too thin.');
          return false;
        }
    ...
    async function loadDetail() {
      ...
      detailRows.push(summaryRow('VerifAI Case ID', asTextCell(statusItem.verifai_case_id || '-')));
      detailRows.push(summaryRow('VerifAI Queue Status', asTextCell(statusItem.verifai_status || '-')));
      detailRows.push(summaryRow('VerifAI Queue Stage', asTextCell(statusItem.verifai_stage || '-')));
      detailRows.push(summaryRow('VerifAI Queued At', asTextCell(formatDateTime(statusItem.verifai_queued_at || ''))));
      const sendBackReason = String((statusItem && statusItem.send_back_reason) || (statusItem && statusItem.auditor_learning) || '').trim();
      if (sendBackReason) {
        detailRows.push(
          summaryRow(
            'Reason for Send Back',
            '<span class="case-detail-rejection-reason">' + escapeHtml(sendBackReason).replace(/\n/g, '<br>') + '</span>'
          )
        );
      }
      ...
      if (String(currentStatusItem.verifai_case_id || '').trim()) {
        ...
        try {
          await refreshCurrentVerifaiReport({ keepPreviousOnError: false });
          appendLog('VerifAI doctor bucket loaded.');
        } catch (err) {
          currentVerifaiReport = null;
          appendLog('VerifAI doctor bucket not ready: ' + String(err && err.message ? err.message : err));
        }
      }

      if (!hasUsableVerifaiAiInput()) {
        latestGeneratedReportHtml = '';
    ...
    reportBtn.addEventListener('click', async function () {
      let reportTab = null;
      const startedAt = Date.now();
      let stageText = 'Checking VerifAI JSON...';
      ...
      try {
        ...
        if (!hasVerifaiReportJsonReady() && !hasExtractedData) {
          ...
          const waitResult = await waitForVerifaiReportJson({
            ...
            onWait: function (info) {
              stageText = 'Waiting for VerifAI JSON... attempt ' + String(info.attempt);
              renderProgress();
            },
          });
          if (waitResult.ready) {
            appendLog('VerifAI JSON received after waiting ' + formatElapsedClock(waitResult.elapsedMs) + '. Continuing report generation.');
          } else {
            const noJsonMsg = 'VerifAI JSON is still not available. Please try again in a bit.';
            setMessage('case-detail-msg', 'err', noJsonMsg);
            appendLog('Generate Report stopped after waiting ' + formatElapsedClock(waitResult.elapsedMs) + '.');
            if (reportTab && !reportTab.closed) renderReportLoadingTab(reportTab, 'VerifAI JSON still not ready.', formatElapsedClock(waitResult.elapsedMs));
            showNoAiInputModal('No AI input was received from VerifAI for this claim yet. Please rerun extraction in VerifAIengine and try again.');
            try {
        ...
        if (aiInputState.thin && !hasExtractedData) {
          ...
          showNoAiInputModal(aiInputState.reason);
          try {
            if (reportTab && !reportTab.closed) reportTab.close();
          } catch (_closeErr) {
          }
    ...
    statusBtn.addEventListener('click', async function () {
      ...
      try {
        await refreshCurrentVerifaiReport({ keepPreviousOnError: true, syncActions: false });
        appendLog('Latest VerifAI JSON refreshed before status update.');
      } catch (refreshErr) {
        appendLog('Latest VerifAI JSON refresh before status update failed: ' + String(refreshErr && refreshErr.message ? refreshErr.message : refreshErr));
      }
      if (!hasVerifaiReportJsonReady()) {
        setActionDisabled(true);
        setMessage('case-detail-msg', '', 'Waiting for VerifAI JSON before marking completed...');
        appendLog('VerifAI JSON is not ready yet. Waiting before marking completed.');
        const waitResult = await waitForVerifaiReportJson({ timeoutMs: 120000, intervalMs: 5000 });
        if (!waitResult.ready) {
          setActionDisabled(false);
          setMessage('case-detail-msg', 'err', 'VerifAI JSON is still not available. Please try again in a bit.');
          appendLog('Mark Completed stopped after waiting ' + formatElapsedClock(waitResult.elapsedMs) + '.');
          showNoAiInputModal('No AI input was received from VerifAI for this claim yet. Please rerun extraction in VerifAIengine and try again.');
          return;
        }
        appendLog('VerifAI JSON received after waiting ' + formatElapsedClock(waitResult.elapsedMs) + '. Continuing status update.');
      }
      const markCompletedAiInputState = getVerifaiAiInputState();
      if (markCompletedAiInputState.thin) {
        setActionDisabled(false);
        setMessage('case-detail-msg', 'err', 'No AI input available from VerifAI.');
        appendLog('Mark Completed stopped because VerifAI input is missing or too thin.');
        showNoAiInputModal(markCompletedAiInputState.reason);
        return;
      }
  ...
  async function renderUploadDocument() {
    ...
    contentPanel.innerHTML = '<section class="claim-status-panel">'
      ...
      + '<th>ACTION</th>'
      + '</tr></thead><tbody id="upload-doc-tbody"><tr><td colspan="10">Loading...</td></tr></tbody></table></div>'
      + '<div class="claim-pagination">'
    ...
    async function queueAiBatch(claimUuid, buttonEl) {
      ...
      try {
        const result = await apiFetch('/api/v1/integrations/verifai/claims/' + encodeURIComponent(claimKey) + '/queue', {
          method: 'POST'
        });
        queuedOk = true;
        setMessage(
          'upload-doc-list-msg',
          'ok',
          'Queued to VerifAI. Case ID: ' + String((result && result.verifai_case_id) || '-')
            + ', documents: ' + String((result && result.documents_sent) || 0)
            + ', status: ' + String((result && result.verifai_status) || '-')
        );
      } catch (err) {
        setMessage('upload-doc-list-msg', 'err', err && err.message ? err.message : 'Queue AI Batch failed.');
      } finally {
        if (btn) {
          btn.disabled = queuedOk;
          btn.textContent = queuedOk ? 'Queued to VerifAI' : (oldText || 'Queue AI Batch');
        }
        await loadRows(false);
      }
    }
    ...
    async function loadRows(resetPage) {
      ...
      const rows = (result.items || []).map((c) => {
        ...
        const isQueuedToVerifai = Boolean(c.verifai_queued || String(c.verifai_case_id || '').trim());
        const verifaiJsonState = String(c.verifai_json_state || (isQueuedToVerifai ? 'waiting' : 'not_sent')).trim().toLowerCase();
        const verifaiJsonLabel = verifaiJsonState === 'received'
          ? 'Received'
          : (verifaiJsonState === 'waiting' ? 'Waiting' : 'Not Sent');
        const verifaiJsonMeta = [String(c.verifai_status || '').trim(), String(c.verifai_stage || '').trim()].filter(Boolean).join(' / ');
        const verifaiJsonCell = statusChip(verifaiJsonLabel)
          + (verifaiJsonMeta ? ('<div class="muted" style="margin-top:4px;">' + escapeHtml(verifaiJsonMeta) + '</div>') : '');
        const canQueue = documentsCount > 0 && !isQueuedToVerifai;
        ...
        const queueLabel = isQueuedToVerifai ? 'Queued to VerifAI' : 'Queue AI Batch';
        return '<tr>'
          + '<td>' + escapeHtml(c.external_claim_id || '-') + '</td>'
          + '<td>' + escapeHtml(treatmentType || '-') + '</td>'
          ...
          + '<td><div class="doctor-case-actions">'
            ...
            + '<button type="button" class="btn-soft" data-queue-claim="' + escapeHtml(c.id || '') + '"' + (canQueue ? '' : ' disabled') + '>' + escapeHtml(queueLabel) + '</button>'
          + '</div></td>'
          + '</tr>';
      }).join('');

      tbody.innerHTML = rows || '<tr><td colspan="10">No claims found for selected filter.</td></tr>';
  ...
  async function renderCompletedReports(initialStatusFilter) {
    ...
    contentPanel.innerHTML = '<section class="claim-status-panel">'
      ...
      + '<div class="table-wrap claim-status-table-wrap"><table><thead><tr>'
      + '<th>CLAIM ID</th><th>DOCTOR</th><th>COMPLETED AT</th><th>REPORT STATUS ^</th><th>QC</th><th>RATING</th><th>TAGGING</th><th>SUBTAGGING</th><th>OPINION</th><th>AI REPORTS</th><th>ACTION</th>'
      + '</tr></thead><tbody id="completed-reports-tbody"><tr><td colspan="11">Loading...</td></tr></tbody></table></div>'
      + '<div class="claim-pagination">'
    ...
    function formatRatingText(value) {
      ...
      if (n <= 0) return '-';
      return String(n) + (n === 1 ? ' star' : ' stars');
    }

    function buildRatingOptions(selectedValue) {
      const current = normalizeRatingValue(selectedValue);
      let html = '<option value="0">Select</option>';
      for (let i = 1; i <= 5; i += 1) {
        html += '<option value="' + String(i) + '"' + (i === current ? ' selected' : '') + '>' + String(i) + (i === 1 ? ' star' : ' stars') + '</option>';
      }
      return html;
    }
    ...
    async function loadRows(resetPage) {
      ...
      const rows = items.map(function (r) {
        ...
        const isQcDone = String(r.qc_status || 'no').toLowerCase() === 'yes';
        const qcText = isQcDone ? 'Yes' : 'No';
        const currentRating = normalizeRatingValue(r.auditor_rating);
        ...
        const ratingLocked = auditOnlyRole && isLockedForAudit;
        const ratingLockTitle = ratingLocked && auditLockTitle ? (' title="' + escapeHtml(auditLockTitle) + '"') : '';
        const ratingCellHtml = (canAuditQc && isQcDone)
          ? ('<div class="action-row action-row-primary">'
            + '<select data-completed-rating-select="' + escapeHtml(claimUuid) + '"'
            + (ratingLocked ? ' disabled' : '')
            + ratingLockTitle
            + '>'
            + buildRatingOptions(currentRating)
            + '</select>'
            + '<button type="button" class="btn-soft" data-completed-save-rating="' + escapeHtml(claimUuid) + '"'
            + (ratingLocked ? ' disabled' : '')
            + ratingLockTitle
            + '>Save</button>'
            + '</div>')
          : (isQcDone
            ? escapeHtml(formatRatingText(currentRating))
            : '<span class="muted">After Save + Mark QC Done</span>');
        return '<tr>'
      ...
      tbody.innerHTML = rows || '<tr><td colspan="11">No completed reports found.</td></tr>';
      ...
      tbody.querySelectorAll('button[data-completed-save-rating]').forEach(function (btn) {
        btn.addEventListener('click', async function () {
          const claimUuid = String(this.getAttribute('data-completed-save-rating') || '').trim();
          const row = rowsByClaim.get(claimUuid);
          if (!row || !claimUuid) return;
          if (auditOnlyRole && isAuditorClaimLocked(row)) {
            setMessage(
              'completed-reports-msg',
              'err',
              'Allotment-wise audit is enabled. Please complete allotment date '
          ...
          const selectEl = tbody.querySelector('select[data-completed-rating-select="' + claimUuid + '"]');
          const rating = Number(selectEl && selectEl.value ? selectEl.value : 0);
          if (!Number.isFinite(rating) || rating < 1 || rating > 5) {
            setMessage('completed-reports-msg', '', 'Please select rating between 1 and 5.');
            return;
          }
          ...
          try {
            const result = await apiFetch('/api/v1/user-tools/completed-reports/' + encodeURIComponent(claimUuid) + '/auditor-rating', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ rating: rating }),
            });
            const claimLabel = String(result && result.external_claim_id ? result.external_claim_id : (row.external_claim_id || '-'));
            setMessage('completed-reports-msg', 'ok', 'Auditor rating updated for claim ' + claimLabel + '.');
            await loadRows(false);
          } catch (err) {
            setMessage('completed-reports-msg', 'err', err && err.message ? err.message : 'Failed to update auditor rating.');
          } finally {
            btn.disabled = false;
    ...
    try {
      ...
      setMessage('completed-reports-msg', 'err', err && err.message ? err.message : 'Failed to load completed reports.');
      tbody.innerHTML = '<tr><td colspan="11">Failed to load completed reports.</td></tr>';
      updatePaginationUi();
    }
  }
...
async function renderLegacyMigration() {
  ...
  async function renderPage(activeRole, page) {
    ...
    if (page === 'ratings') return renderDoctorRatings();
    if (page === 'doctor-ratings') return renderUserDoctorRatings();
