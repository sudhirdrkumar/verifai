...
def _run_legacy_migration_job(job_id: str) -> None:
    ...
    try:
        ...
        if include_users:
            # User sync intentionally disabled for legacy migration.
            progress["users"] = {
                "candidates": 0,
                "created": 0,
                "updated": 0,
            ...
            _update_job(job_id, progress=progress, message="User sync is disabled; claims-only migration continues")
...
def start_legacy_migration(
    ...
    if not payload.include_claims:
        raise HTTPException(status_code=400, detail="Enable include_claims")
...
def delete_medicine(
    ...
    db.commit()
    return {"id": int(row["id"]), "message": "medicine deleted"}
